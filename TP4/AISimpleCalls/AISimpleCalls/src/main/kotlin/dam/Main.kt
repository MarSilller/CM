...
fun main() = runBlocking {
    ...
    while (true) {
        ...
        println("Choose mode:")
        println("1. Simple input processing (Chat)")
        println("2. Sentiment Analysis")
        print("Your choice (1/2, or press Enter for default Chat): ")
        val modeStr = readlnOrNull() ?: break
        val isSentimentMode = modeStr.trim() == "2"

        // Ask for question input and read it from the console
        print(if (isSentimentMode) "🧠 Enter text for sentiment analysis: " else "🧠 Your question: ")
        ...
        // Process input
        try {
            val output = if (isSentimentMode) {
                assistant.processSentimentAnalysis(input)
            } else {
